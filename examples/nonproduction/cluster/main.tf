...
data "terraform_remote_state" "egress" {
  backend = "oss"
  config = merge(local.remote_backend, {
    prefix = "platform-foundation/nonproduction-egress"
  })
}

module "logging" {
  source            = "../../../modules/ack-logging"
  project_name      = "example-nonprod-ack"
...
module "cluster" {
  ...
  resource_group_id         = data.terraform_remote_state.resource_groups.outputs.example_nonprod_id
...
module "system_a" {
  ...
  vswitch_ids       = [data.terraform_remote_state.network.outputs.subnet_ids_by_name["nonprod-node-a"]]
...
module "system_b" {
  ...
  name              = "system-b"
  cluster_id        = module.cluster.cluster_id
  vswitch_ids       = [data.terraform_remote_state.network.outputs.subnet_ids_by_name["nonprod-node-b"]]
...
module "system_c" {
  ...
  name              = "system-c"
  cluster_id        = module.cluster.cluster_id
  vswitch_ids       = [data.terraform_remote_state.network.outputs.subnet_ids_by_name["nonprod-node-c"]]
